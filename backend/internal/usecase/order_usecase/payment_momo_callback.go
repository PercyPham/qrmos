...
func (u *MoMoPaymentCallbackUsecase) HandleCallback(t time.Time, data *momo.PaymentCallbackData) (*entity.Order, error) {
	if err := data.Verify(); err != nil {
		return nil, apperror.Wrap(err, "verify momo ipn callback data")
	}
	orderID, err := data.GetQrmosOrderID()
	if err != nil {
		return nil, apperror.Wrap(err, "get order id from ipn callback request data")
	}
	order := u.orderRepo.GetByID(orderID)
	if order == nil {
		return nil, apperror.Wrap(err, "order not found")
	}
	...
	if order.State != entity.OrderStatePending {
		if order.State == entity.OrderStateConfirmed {
			...
			return nil, apperror.Newf("duplicate order payment for order '%d', new momo payment request id is '%s'", order.ID, data.RequestID)
		}
		return nil, apperror.Newf("unexpected payment for '%s' order '%d' with momo payment request id '%s'", order.State, order.ID, data.RequestID)
	}

	if err := order.MarkPaidByMoMo(t, data.RequestID, data.TransID); err != nil {
		return nil, apperror.Wrap(err, "mark order as paid by momo")
	}
	if err := u.orderRepo.Update(order); err != nil {
		return nil, apperror.Wrap(err, "repo updates order")
	}
	return order, nil
}
